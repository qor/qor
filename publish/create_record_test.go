...
func TestCreateStructFromDraftWithPublishImmediately(t *testing.T) {
	name := "create_product_from_draft_with_publish_immediately"
	pbdraft.Set("publish:publish_immediately", true).Create(&Product{Name: name, Color: Color{Name: name}})

	if pbprod.First(&Product{}, "name = ?", name).RecordNotFound() {
		t.Errorf("record should be found in production db when using publish immediately")
	}
...
func TestCreateStructFromProductionWithPublishImmediately(t *testing.T) {
	name := "create_product_from_production_with_publish_immediately"
	pbprod.Set("publish:publish_immediately", true).Create(&Product{Name: name, Color: Color{Name: name}})
